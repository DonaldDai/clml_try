...
n_layer=5
n_dim=600
# continue_epoch 只有接着之前epoch跑的时候才要改
# c_epoch=9
# 创建ckpt目录
mkdir -p "./ckpts/mamba${c_epoch}_${n_layer}_${n_dim}"
# 执行PyTorch分布式训练命令
...
srun bash -c "torchrun --nproc_per_node=3 --nnodes=8 --node_rank=\$SLURM_NODEID --master_addr=$MASTER_ADDR --master_port=29500 pretrain_modify_mamba_params.py --batch_size 3 --n_layer=$n_layer --n_dim=$n_dim --data_path='./data/panglao_human.h5ad' --ckpt_path='' --ckpt_dir='./ckpts/mamba${c_epoch}_${n_layer}_${n_dim}' 2>&1"
